...
export default function RegionError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error('Region page failed to render', error);
  }, [error]);
  ...
  return (
    <main className="flex min-h-[60vh] items-center justify-center px-6 py-14 sm:px-10 lg:px-16">
      <div className="w-full max-w-md rounded-[2rem] border border-slate-200 bg-white p-10 text-center shadow-glow">
        <p className="text-lg font-semibold text-slate-900">We couldn&apos;t load this region right now.</p>
