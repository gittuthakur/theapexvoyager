...
export default function JourneysError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error('Journeys page failed to render', error);
  }, [error]);
  ...
  return (
    <main className="flex min-h-[60vh] items-center justify-center px-6 py-14 sm:px-10 lg:px-16">
      <div className="w-full max-w-md rounded-[2rem] border border-slate-200 bg-white p-10 text-center shadow-glow">
        <p className="text-lg font-semibold text-slate-900">We couldn&apos;t load journeys right now.</p>
        <p className="mt-3 text-sm text-slate-600">Something went wrong loading the journeys catalog. Please try again in a moment.</p>
        <button
          type="button"
          onClick={reset}
